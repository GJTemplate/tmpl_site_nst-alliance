			...
			switch ($type) {
				...
				case 'Z':
					...
					if ($category = $db->loadObject ()) {
						...
						$customfield->display = '<div class="spacer"><div class="category-image card">'.JHtml::link (JRoute::_ ('index.php?option=com_virtuemart&view=category&virtuemart_category_id=' . $category->virtuemart_category_id), $thumb, array('title' => $category->category_name)).'</div><div class="category-name">'.JHtml::link (JRoute::_ ('index.php?option=com_virtuemart&view=category&virtuemart_category_id=' . $category->virtuemart_category_id), $category->category_name).'</div>'.'</div>';
					}
