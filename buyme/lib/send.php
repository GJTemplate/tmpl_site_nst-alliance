...
//require("smtp.php");
	...
	# User Id coma separator
	use Joomla\CMS\Factory;
	
	
	# Массив пользовотелей
	$USER_ID_RECIPIENT_ARR = [ 408 , 409 ] ;
...
header ("Content-Type: text/html; charset=utf-8"); //кодировка
	...
	define('USER_ID_RECIPIENT_ARR', $USER_ID_RECIPIENT_ARR);
	
	function sendMail ( $body )
	{
		$subject = 'Новый заказ' ;
		$userRecipient_arr = USER_ID_RECIPIENT_ARR ;
		$config = Factory::getConfig();
		$mailer = Factory::getMailer();
		$mailer->setSender( [ $config->get( 'mailfrom' ), $config->get( 'name' ) ] );
		foreach (  $userRecipient_arr as $user_id)
		{
...
$l["footer"] = "<div style=\"background: #f1f1f1; margin-top: 10px; padding: 10px; color: #555;\"></div>";
...
if ($interval < 20) { // если прошло менее (сек)
	...
	if (count($get_data) > 1) { // data to send
		...
		$mess = "<h3>Заказ «".$prd."»</h3>
		<div style='background:#ffffff;border:1px solid #abc9ff;padding:10px 20px'>";

		$mess .= getOptions(1);
		
		$mess .= "</div>
		
		<div style=\"background: #f9f2f4; border: 1px solid #f9f2f4;padding:10px 20px;margin-top:10px;\">";
		...
		if (strlen($to) > 5 && $to != /*"yourMail"*/"master@artos.com.ua") {
